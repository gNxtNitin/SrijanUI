...
$(document).ready(() => {
    ...
    const epunchReport_table = $('.datatables-epunchreport').DataTable({
        ajax: {
            url: `${baseUrl}/report/GetTeamEpunchReportData`,
            data: {
                EmpId: '',
                DTRangeFrom: '',
                DTRangeTo: '',
    ...
    $('.card').on('click', '#applyDateFilter', function () {
        ...
        const newUrl = `${baseUrl}/report/GetTeamEpunchReportData?EmpId=${empId}&DTRangeFrom=${encodeURIComponent(fromDate)}&DTRangeTo=${encodeURIComponent(toDate)}&IsTeamData=${encodeURIComponent(isTeamData)}`;
